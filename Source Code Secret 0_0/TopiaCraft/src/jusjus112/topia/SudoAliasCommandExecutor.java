...
public class SudoAliasCommandExecutor
      ...
      if (args.length != 0) {
        if (args.length == 1) {
          if ((args[0].equalsIgnoreCase("reload")) && (sender.hasPermission("SudoAlias.reload"))) {
            ...
            return true;
          }if (args[0].equalsIgnoreCase("nothing"))
            return true;
        }
      }
      else {
        PluginDescriptionFile pdf = SudoAlias.getInstance().getDescription();
        sender.sendMessage(ChatColor.AQUA + "SudoAlias " + ChatColor.YELLOW + "v" + pdf.getVersion() + ChatColor.AQUA + " by " + ChatColor.YELLOW + (String)pdf.getAuthors().get(0) + ChatColor.AQUA + " is " + ChatColor.GREEN + "running");
        return true;
      }
    }
    return false;
